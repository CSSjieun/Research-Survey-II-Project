...
```{r}
library(dplyr)
unemployment_cnty <- read.csv("./Data/API_SL.UEM.TOTL.ZS_DS2_en_csv_v2_80.csv", skip = 4, header = TRUE)
unemployment_cnty <- unemployment_cnty |> select("Country.Name", "X2019")

unemployment_cnty <- unemployment_cnty |> rename("country" = "Country.Name")
unemployment_cnty <- unemployment_cnty |> rename("unemployement_rate" = "2019")

survey2 |> left_join(unemployment_cntry)
```
...
```{r}
library(readr)
political_regime <- read_csv("Data/political-regime (1).csv")
View(political_regime_1_)
```
...
political_regime1 %>%
  select(Entity, `Political regime`, pol_regimen)
```

## Merging survey2 and political_regime1

```{r}
survey2 <- left_join(survey2, political_regime1, by = c("country" = "Entity")) 
```

### 1. Encoding **dependent** variable: qc19
