...
factors. These independent variables are:
  ...
  - Generic individual aspects:
    ...
    - *EDUCATION: (d8r1)* - EXCLUDED
    - RELIGION: sd3 (then: religion and religion_group)
    - POLITICAL STANCE: d1r1 (then: left_right)
    - UNEMPLOYED: d15a_r1 (then: unemployed) 
    - LIFE SATISFACTION: d70 (then: Life_satisf)
    
  - LGTBI related aspects at individual level:
    - CONTACT: 
      - sd1_4 (gay lesbian or bisexual, then: contact_gay)
      - sd1_7 (transgender/transexual, then: contact_trans)
      - sd1_8 (intersext, then: contact_inter)
    - DISCRIMINATION PERCEIVED in country (LGTBI):
      - qc1_4 DISCRIMINATION IN CTRY: SEXUAL ORIENTATION (then: disc_cntry_sexual)
      - qc1_8 DISCRIMINATION IN CTRY: BEING TRANSGENDER (then: disc_cntry_trans)
      - qc1_10 DISCRIMINATION IN CTRY: BEING INTERSEX (then: disc_cntry_inter)
    - ATTITUDES TOWARDS THE LGTBI COMMUNITY:
      - qc15_1 (then: att_rights_sexual)
    - EXPERIENCES OF DISCRIMINATION (LGTBI)
      - qc2t_3  EXPERIENCE DISCRIMINATION: SEXUAL ORIENTATION (SUM) (then: disc_exp)
...
### 5-3. Religion (Independent Variable) - Discrimination_perceived

Encoding religion

### 5-4. Religion (Independent Variable) - Discrimination experienced

Encoding religion

### 5-5. Religion (Independent Variable) - Contact with LGBT community
...
political_regime1$Entity <- gsub("Czechia", "Czech Republic", political_regime1$Entity)
```

```{r}
political_regime1 <- political_regime1 %>% 
  mutate(pol_regimen = case_when(
    `Political regime` == 1 ~ "electoral autocracies",
    `Political regime` == 2 ~ "electoral democracies", 
    `Political regime` == 3 ~ "liberal democracies",
    TRUE ~ NA_character_  
  )) %>%
...
# Merging survey2 and political_regime1
survey <- left_join(survey, political_regime1, by = c("country" = "Entity")) 
...
mice_imputed <- data.frame(
  original = Disc_clean$change_docs,
