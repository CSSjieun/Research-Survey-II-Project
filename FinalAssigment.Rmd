...
Disc_clean <- Disc %>% 
  select(country, d10, d11, d8r1, d1r1, sd3, sd1_4, sd1_7, sd1_8, qc19, qc20, d70, d15a_r1)
```
...
Disc_clean$male <- ifelse(Disc_clean$male == "No", 0, 
                                 ifelse(Disc_clean$male== "Yes", 1, NA))
...
Disc_clean$contact_gay <- ifelse(Disc_clean$contact_gay == "No", 0, 
                                 ifelse(Disc_clean$contact_gay== "Yes", 1, NA))
...
Disc_clean$contact_trans <- ifelse(Disc_clean$contact_trans == "No", 0, 
                                 ifelse(Disc_clean$contact_trans== "Yes", 1, NA))
...
Disc_clean$contact_inter <- ifelse(Disc_clean$contact_inter == "No", 0, 
                                 ifelse(Disc_clean$contact_inter== "Yes", 1, NA))
...
```{r}
library(readxl)
rainbow_2019 <- read_excel("rainbow-2019.xlsx")
...
```{r}
rainbow_2019 <- rainbow_2019 %>% 
  mutate(constitution_gender_identity = case_when(
    `Constitution (gender identity) ()` == 1 ~ "national/federal application",
    `Constitution (gender identity) ()` == 2 ~ "applicable is some regions only", 
    `Constitution (gender identity) ()` == 3 ~ "No applications",
    TRUE ~ NA_character_  
  )) %>%
  mutate(constitution_gender_identity = factor(constitution_gender_identity,
                              levels = c("national/federal application", "applicable is some regions only", "No applications")))

```

```{r}
rainbow_2019 <- rainbow_2019 %>% 
  mutate(employment_gender_identity = case_when(
    `Employment (gender identity) ()` == 1 ~ "national/federal application",
    `Employment (gender identity) ()` == 2 ~ "applicable is some regions only", 
    `Employment (gender identity) ()` == 3 ~ "No applications",
    TRUE ~ NA_character_  
  )) %>%
  mutate(employment_gender_identity = factor(employment_gender_identity,
                              levels = c("national/federal application", "applicable is some regions only", "No applications")))
```

```{r}
rainbow_2019 <-rainbow_2019 %>% 
  mutate(education_gender_identity = case_when(
    `Education (gender identity) ()` == 1 ~ "national/federal application",
    `Education (gender identity) ()` == 2 ~ "applicable is some regions only", 
    `Education (gender identity) ()` == 3 ~ "No applications",
    TRUE ~ NA_character_  
  )) %>%
  mutate(education_gender_identity = factor(education_gender_identity,
                              levels = c("national/federal application", "applicable is some regions only", "No applications")))
```

```{r}
rainbow_2019 <- rainbow_2019 %>% 
  mutate(LGBTI_human_rights_not_risk = case_when(
    `LGBTI human rights defenders are not at risk ()` == 1 ~ "national/federal application",
    `LGBTI human rights defenders are not at risk ()` == 2 ~ "applicable is some regions only", 
    `LGBTI human rights defenders are not at risk ()` == 3 ~ "No applications",
    TRUE ~ NA_character_  
  )) %>%
  mutate(LGBTI_human_rights_not_risk = factor(LGBTI_human_rights_not_risk,
                              levels = c("national/federal application", "applicable is some regions only", "No applications")))
...
```{r}
library(dplyr)
unemployment_cntry <- read.csv("./API_SL.UEM.TOTL.ZS_DS2_en_csv_v2_80.csv", skip = 4, header = TRUE)
...
```{r}
sapply(survey_n_simple, function(x) sum(is.na(x))*100/nrow(survey_n_simple))

# categorical variable
survey_c = survey |> select(country, male, age_cat, left_right, religion, religion_group, Life_satisf)
table(is.na(survey_c))
# numerical variable
survey_n = survey |> select(-country, -male, -age_cat, -left_right, -religion, -religion_group, -Life_satisf)
# we separated two dataset since the scale of data in those two dataset are too different
survey_n_simple <- survey_n |> select(constitution_rb, employment_rb, education_rb, defenders_at_risk_rb)
survey_n_complex <- survey_n |> select(GDP_per_capita, all_religion, score_rb, unemployement_rate)
survey_n_dummy <- survey_n |> select(unemployed, contact_gay, contact_trans, contact_inter, change_docs)

md.pattern(survey_n_simple)
md.pattern(survey_n_complex)
...
# survey_n_dummy
mice_mod_d = mice(survey_n_dummy, m = m, method='cart', seed = 123) # machine learning tool, rf = random forest
survey_n_d_imputed <- complete(mice_mod_d, action=m) 

colSums(is.na(survey_n_d_imputed))

# survey_n_complex
mice_mod_c = mice(survey_n_complex, m = m, method='rf', seed = 2345) # machine learning tool, rf = random forest
survey_n_c_imputed <- complete(mice_mod_c, action=m) 

colSums(is.na(survey_n_c_imputed))
```

## Data frame

```{r}
survey_n_imputed <- cbind(survey_n_s_imputed, survey_n_d_imputed, survey_n_c_imputed)
...
```{r}
LR <- glm(change_docs ~. , data = survey, family = "binomial")
```
